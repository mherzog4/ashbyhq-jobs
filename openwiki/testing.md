...
| SQLite history | `test_db_upsert_preserves_history`, `test_db_keeps_grep_context_from_earlier_runs`, `test_migrates_a_pre_multi_ats_database`, `test_migrates_a_database_created_before_closed_at`, `test_unfiltered_run_closes_vanished_postings`, `test_filtered_run_never_closes_anything`, `test_closing_is_scoped_to_boards_actually_scanned`, `test_db_skips_rows_without_an_id` | `first_seen` preservation, `last_seen` refresh, matched-context retention, pre-multi-ATS and pre-`closed_at` migrations, closed-posting lifecycle, coverage scoping, id requirement. |
