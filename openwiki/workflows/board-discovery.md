...
`load_boards(refresh, ats_list, concurrency)` checks sources in this order:

1. If `--refresh-boards` is not set, merge `/boards.seed.json` and generated `boards.json` by platform. Later files win where they contain entries, so a generated cache can override a seed for one ATS without erasing seed-only platforms.
...
3. Otherwise, or when `--refresh-boards` is set, call `discover_boards(ats)` for each selected platform and write the generated cache to `boards.json`.
...
flowchart TD
    ...
    Domains --> Wayback["query Wayback CDX"]
...
When changing this workflow, update [testing](../testing.md) fixtures for URL parsing, JSONL parsing, plausible slug filtering, per-ATS source behavior, and `HEAD` validation. When changing cache semantics, update the [data model](../architecture/data-model.md) only if coverage or closing behavior changes, and update the [operations runbook](../operations/runbook.md) if git hygiene or seed guidance changes.
